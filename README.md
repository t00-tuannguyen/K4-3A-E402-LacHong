...
**Lớp:** 3A · **Phòng:** E402 · **Cụm:** C2 · **Track:** B
...
| Trần Phạm Thái Vũ | 2A202602695 | Data & Evaluation Lead | Mining `k4_messages.csv` lấy bằng chứng định lượng + ≥5 quote; taxonomy 4 lớp chỗ khó; Dev Set + Sealed Eval Set (`eval/`) và script đo `eval/run_eval.py`; viết `spec.md` §5, §7; cùng Võ Phú Hãn chạy validation; thuyết trình Slide 4 |
...
> Quy tắc evaluation của nhóm: **không được sửa rules dựa trên kết quả `eval/eval_set.json`**. Chỉ dùng `eval/dev_set.json` trong quá trình phát triển; xem `eval/README.md` để biết cách chạy và phân bổ case.
